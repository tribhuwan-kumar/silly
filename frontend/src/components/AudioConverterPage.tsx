...
export function AudioConverterPage() {
    ...
    setFiles((prev) => {
      ...
      if (paths.length > 0 && m4aFiles.length === 0) {
        toast.info("No new files added", {
          description:
            "All files were already added or have unsupported format",
        });
      }
      return prev;
    });
  ...
  return (
    <div className={`space-y-6 ${isFullscreen ? "h-full flex flex-col" : ""}`}>
      ...
      <div
        className={`flex flex-col items-center justify-center border-2 border-dashed rounded-lg transition-all ${isFullscreen ? "flex-1 min-h-[400px]" : "h-[400px]"} ${
          isDragging
