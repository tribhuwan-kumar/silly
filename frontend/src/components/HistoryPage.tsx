...
export function HistoryPage({ onHistorySelect }: HistoryPageProps) {
  ...
  const renderDownloadHistory = () => {
    const totalPages = Math.ceil(
      filteredDownloadHistory.length / ITEMS_PER_PAGE,
    );
    const startIndex = (downloadCurrentPage - 1) * ITEMS_PER_PAGE;
    const paginated = filteredDownloadHistory.slice(
      startIndex,
                    ...
                    <td className="p-3 align-middle text-left hidden lg:table-cell">
                      <div className="flex flex-col items-start gap-1">
                        <span className="text-xs font-bold text-foreground">
                          {["HI_RES_LOSSLESS", "LOSSLESS"].includes(item.format)
                            ? "FLAC"
                            : item.format}
                        </span>
                        {item.quality && (
                          ...
                          <Tooltip delayDuration={0}>
                            <TooltipTrigger asChild>
                              <Button
                                ...
                                onClick={() =>
                                  handlePreview(item.id, item.spotify_id)
                                }
                          ...
                          <Tooltip delayDuration={0}>
                            <TooltipTrigger asChild>
                              <Button
                                ...
                                onClick={() =>
                                  handleDeleteDownloadItem(item.id)
                                }
                              >
        ...
        {totalPages > 1 && (
          <Pagination>
            <PaginationContent>
              ...
              {getPaginationPages(downloadCurrentPage, totalPages).map(
                (page, index) =>
                  page === "ellipsis" ? (
  ...
  const renderFetchHistory = () => {
    ...
    const paginated = filteredFetchHistory.slice(
      startIndex,
      startIndex + ITEMS_PER_PAGE,
    );
    return (
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center gap-2">
              <div className="relative flex-1">
                <Search className="absolute left-2 top-2.5 h-4 w-4 text-muted-foreground" />
                <Input
                  placeholder="Search fetch history..."
                          ...
                          <Tooltip delayDuration={0}>
                            <TooltipTrigger asChild>
                              <Button
                                ...
                                className="h-8 w-8 cursor-pointer text-destructive hover:text-destructive"
                                onClick={(e) =>
                                  handleDeleteFetchItem(item.id, e)
                                }
                              >
                                <Trash2 className="h-4 w-4" />
                              </Button>
        ...
        {totalPages > 1 && (
          <Pagination>
            <PaginationContent>
              ...
              {getPaginationPages(fetchCurrentPage, totalPages).map(
                (page, index) =>
                  page === "ellipsis" ? (
                    <PaginationItem key={`ellipsis-${index}`}>
                      <PaginationEllipsis />
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border-b">
        <div className="flex gap-6">
          <button
            ...
            className={`pb-3 text-sm font-medium transition-colors border-b-2 -mb-px hover:text-foreground ${activeTab === "downloads" ? "border-primary text-foreground" : "border-transparent text-muted-foreground"}`}
          >
            Downloads
          </button>
          <button
            onClick={() => setActiveTab("fetches")}
            className={`pb-3 text-sm font-medium transition-colors border-b-2 -mb-px hover:text-foreground ${activeTab === "fetches" ? "border-primary text-foreground" : "border-transparent text-muted-foreground"}`}
          >
            Fetches
          </button>
        </div>
      ...
      {activeTab === "downloads" && (
        <div className="mt-6">{renderDownloadHistory()}</div>
      )}

      {activeTab === "fetches" && (
        <div className="mt-6">{renderFetchHistory()}</div>
      )}

      <Dialog
        open={showClearDownloadConfirm}
        onOpenChange={setShowClearDownloadConfirm}
      >
        <DialogContent className="max-w-md [&>button]:hidden">
          <DialogHeader>
            <DialogTitle>Clear Download History?</DialogTitle>
            <DialogDescription>
              This will remove all entries from your download history. This
              action cannot be undone. Note: The actual downloaded files will
              NOT be deleted.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
      ...
      <Dialog
        open={showClearFetchConfirm}
        onOpenChange={setShowClearFetchConfirm}
      >
        <DialogContent className="max-w-md [&>button]:hidden">
          <DialogHeader>
            <DialogTitle>
              Clear{" "}
              {activeFetchTab.charAt(0).toUpperCase() + activeFetchTab.slice(1)}{" "}
              History?
            </DialogTitle>
            <DialogDescription>
              This will remove all {activeFetchTab} entries from your fetch
              history cache.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
