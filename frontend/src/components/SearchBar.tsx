...
const RECENT_SEARCHES_KEY = "silly_recent_searches";
...
export function SearchBar({
  ...
  const isSpotifyUrl = (text: string) => {
    ...
    const isUrl =
      /^(https?:\/\/|www\.)/i.test(trimmed) || /^spotify:/i.test(trimmed);
  ...
  return (
    <div className="space-y-4">
          ...
          {!searchMode ? (
            <>
              <InputWithContext
                ...
                onPaste={handlePaste}
                onKeyDown={(e) =>
                  e.key === "Enter" && handleFetchWithValidation()
                }
                className="pr-8"
              />
              {url && (
          ...
          <>
            <Select value={region} onValueChange={onRegionChange}>
              ...
              <SelectContent className="max-h-[300px]">
                {REGIONS.map((r) => (
                  <SelectItem key={r} value={r} textValue={r}>
                    {r}{" "}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5 min-w-0">
                          ...
                          {track.is_explicit && (
                            <span
                              className="flex items-center justify-center min-w-[16px] h-[16px] rounded bg-red-600 text-[10px] font-bold text-white leading-none shrink-0"
                              title="Explicit"
                            >
                              E
                            </span>
      ...
      <Dialog
        open={showInvalidUrlDialog}
        onOpenChange={setShowInvalidUrlDialog}
      >
        <DialogContent className="sm:max-w-[425px]">
          ...
          {invalidUrl && (
            <div className="p-3 bg-muted rounded-md border text-xs font-mono break-all opacity-70">
              {invalidUrl}
            </div>
          )}
