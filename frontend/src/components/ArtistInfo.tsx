...
export function ArtistInfo({
  ...
  const [downloadingGalleryIndex, setDownloadingGalleryIndex] = useState<
    number | null
  >(null);
  const [downloadingAllGallery, setDownloadingAllGallery] = useState(false);
  const [activeTab, setActiveTab] = useState<"albums" | "tracks" | "gallery">(
    "albums",
  );
  const filteredAlbumGroups = useMemo(() => {
    const albumTypeMap = new Map(albumList.map((a) => [a.name, a.album_type]));
    const albumGroups = trackList.reduce(
      (acc, track) => {
        if (!track.album_name) return acc;
                ...
                <div className="flex gap-6 items-start">
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {artistInfo.verified && (
                        <BadgeCheck className="h-6 w-6 text-white fill-blue-400 shrink-0" />
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-2 text-sm flex-wrap text-white/90">
                      ...
                      <span>
                        ...
                        {artistInfo.followers === 1 ? "follower" : "followers"}
                      </span>
                      {artistInfo.listeners && (
                        <>
                          <span>•</span>
                          <span>
                            {artistInfo.listeners.toLocaleString()}{" "}
                            {artistInfo.listeners === 1
                              ? "listener"
                              : "listeners"}
                          </span>
                        </>
                    ...
                    <div className="flex items-center gap-2 text-sm flex-wrap text-white/90">
                      <span>
                        {albumList.length}{" "}
                        {albumList.length === 1 ? "album" : "albums"}
                      </span>
                      <span>•</span>
                      <span>
                        {trackList.length}{" "}
                ...
                <div className="flex items-center gap-2 text-sm flex-wrap">
                  ...
                  {artistInfo.listeners && (
                    <>
                      ...
                      <span>
                        {artistInfo.listeners.toLocaleString()}{" "}
                        {artistInfo.listeners === 1 ? "listener" : "listeners"}
                      </span>
                    </>
                  )}
                </div>
      ...
      <div className="border-b">
        <div className="flex gap-6">
          <button
            ...
            className={`pb-3 text-sm font-medium transition-colors border-b-2 -mb-px hover:text-foreground ${activeTab === "albums" ? "border-primary text-foreground" : "border-transparent text-muted-foreground"}`}
          >
            Albums
          </button>
          <button
            onClick={() => setActiveTab("tracks")}
            className={`pb-3 text-sm font-medium transition-colors border-b-2 -mb-px hover:text-foreground ${activeTab === "tracks" ? "border-primary text-foreground" : "border-transparent text-muted-foreground"}`}
          >
            All Tracks
          </button>
          {hasGallery && (
            <button
              ...
              className={`pb-3 text-sm font-medium transition-colors border-b-2 -mb-px hover:text-foreground ${activeTab === "gallery" ? "border-primary text-foreground" : "border-transparent text-muted-foreground"}`}
            >
              Gallery
            </button>
          )}
                  ...
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/50 transition-colors flex items-center justify-center">
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          onClick={() =>
                            handleDownloadGalleryImage(imageUrl, index)
                          }
                          size="sm"
                          variant="secondary"
                  ...
                  <ScrollArea className="flex-1 pr-4">
                    <div className="space-y-4">
                      {filteredAlbumGroups.map(([albumName, data]) => {
                        const tracksWithId = data.tracks.filter(
                          (t) => t.spotify_id,
                        );
                        const isSelected =
                          tracksWithId.length > 0 &&
                          tracksWithId.every((t) =>
                            selectedTracks.includes(t.spotify_id!),
                          );
                        return (
                          <div
                            key={albumName}
                          ...
                          >
                            <Checkbox
                              id={`album-select-${albumName}`}
                              checked={isSelected}
                              onCheckedChange={() =>
                                onToggleSelectAll(data.tracks)
                              }
                              className="mt-1"
                            />
                            <div className="grid gap-1.5 leading-none flex-1">
                              <label
                                htmlFor={`album-select-${albumName}`}
                              ...
                              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                ...
                                <span>•</span>
                                <span>
                                  {data.tracks[0]?.release_date?.split(
                                    "-",
                                  )[0] || "Unknown Year"}
                                </span>
                              </div>
