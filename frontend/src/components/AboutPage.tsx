...
export function AboutPage({ version }: AboutPageProps) {
  ...
  const [activeTab, setActiveTab] = useState<
    "bug_report" | "feature_request" | "faq" | "projects" | "support"
  >("bug_report");
...
${contextContent}`;
  ...
  return (
    <div
      className={`flex flex-col space-y-4 ${activeTab === "faq" ? "h-[calc(100vh-10rem)]" : ""}`}
    >
      ...
      <div
        className={`flex-1 min-h-0 ${activeTab === "faq" ? "overflow-hidden" : ""}`}
      >
        {activeTab === "bug_report" && (
          ...
          <div className="p-1 pr-2">
            <div className="grid gap-2 grid-cols-4">
              <div className="flex flex-col gap-2 h-full">
                <Card
                  className="hover:bg-muted/50 hover:border-primary/50 transition-colors cursor-pointer flex-1"
                ...
                <Card
                  className="hover:bg-muted/50 hover:border-primary/50 transition-colors cursor-pointer flex-1"
                  onClick={() => openExternal("https://spotubedl.com/")}
                >
                  <CardHeader>
              ...
              <Card
                className="hover:bg-muted/50 hover:border-primary/50 transition-colors cursor-pointer"
                onClick={() =>
                ...
                {repoStats["SpotiDownloader"] && (
                  <CardContent className="space-y-3">
                    <div className="flex flex-wrap gap-2 text-xs">
                      {repoStats["SpotiDownloader"].languages?.map(
                        (lang: string) => (
                          <span
                            key={lang}
                    ...
                    <div className="flex flex-col gap-1 text-xs text-muted-foreground items-start">
                      <span className="flex items-center gap-1">
                        ...
                        {formatNumber(
                          repoStats["SpotiDownloader"].totalDownloads,
                        )}
                      </span>
                      <span className="flex items-center gap-1 text-green-600 dark:text-green-400">
                        <Download className="h-3.5 w-3.5" /> LATEST:{" "}
                        {formatNumber(
                          repoStats["SpotiDownloader"].latestDownloads,
                        )}
                      </span>
                    </div>
                  </CardContent>
              ...
              >
                ...
                {repoStats["SpotiFLAC-Next"] && (
                  <CardContent className="space-y-3">
                    <div className="flex flex-wrap gap-2 text-xs">
                      {repoStats["SpotiFLAC-Next"].languages?.map(
                        (lang: string) => (
                          <span
                            key={lang}
                            className="px-2 py-0.5 rounded-full font-medium"
                    ...
                    <div className="flex flex-col gap-1 text-xs text-muted-foreground items-start">
                      <span className="flex items-center gap-1">
                        <Download className="h-3.5 w-3.5" /> TOTAL:{" "}
                        {formatNumber(
                          repoStats["SpotiFLAC-Next"].totalDownloads,
                        )}
                      </span>
                      <span className="flex items-center gap-1 text-green-600 dark:text-green-400">
                        <Download className="h-3.5 w-3.5" /> LATEST:{" "}
                        {formatNumber(
                          repoStats["SpotiFLAC-Next"].latestDownloads,
                        )}
                      </span>
                    </div>
                  </CardContent>
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div className="flex items-center gap-4 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1">
                        <Star className="h-3.5 w-3.5 fill-amber-500 text-amber-500" />{" "}
                        {formatNumber(
                          repoStats["Twitter-X-Media-Batch-Downloader"].stars,
                        )}
                      </span>
                      <span className="flex items-center gap-1">
                        <GitFork className="h-3.5 w-3.5" />{" "}
          ...
          <div className="flex flex-col items-center justify-center p-8 space-y-8">
            <div className="text-center space-y-2">
              ...
              <p className="text-muted-foreground max-w-[500px]">
                ...
                supporting the project by buying me a coffee. Your support helps
            ...
            <div className="grid sm:grid-cols-2 gap-4 w-full max-w-lg">
              <Button
                size="lg"
                className="h-16 text-lg font-semibold text-white gap-3 group"
              ...
              <Button
                size="lg"
                className="h-16 text-lg font-semibold text-black gap-3 group"
