...
export function useAudioAnalysis() {
  ...
  const analyzeFile = useCallback(async (filePath: string) => {
    ...
    try {
      ...
      const response = await app.AnalyzeTrack(filePath);
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to analyze audio file";
  ...
  useEffect(() => {
    ...
    const loadSpectrum = () => {
      rafId = requestAnimationFrame(() => {
        ...
        if (cachedSpectrum) {
          setResult((prev) =>
            prev ? { ...prev, spectrum: cachedSpectrum } : null,
          );
          setSpectrumLoading(false);
        } else {
          setSpectrumLoading(false);
