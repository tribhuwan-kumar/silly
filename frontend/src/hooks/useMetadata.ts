...
export function useMetadata() {
  ...
  const [metadata, setMetadata] = useState<SpotifyMetadataResponse | null>(
    null,
  );
  ...
  const [pendingArtistName, setPendingArtistName] = useState<string | null>(
    null,
  );
  ...
  const saveToHistory = async (url: string, data: SpotifyMetadataResponse) => {
    try {
      ...
      if ("track" in data) {
        ...
        image =
          data.track.images && data.track.images.length > 0
            ? data.track.images
            : "";
  ...
  const fetchMetadataDirectly = async (url: string) => {
    ...
    try {
      ...
      if ("track" in data) {
        logger.success(
          `fetched track: ${data.track.name} - ${data.track.artists}`,
        );
        logger.debug(`duration: ${data.track.duration_ms}ms`);
      } else if ("album_info" in data) {
        logger.success(`fetched album: ${data.album_info.name}`);
        logger.debug(
          `${data.track_list.length} tracks, released: ${data.album_info.release_date}`,
        );
      } else if ("playlist_info" in data) {
        logger.success(`fetched playlist: ${data.track_list.length} tracks`);
        logger.debug(
          `by ${data.playlist_info.owner.display_name || data.playlist_info.owner.name}`,
        );
      } else if ("artist_info" in data) {
        logger.success(`fetched artist: ${data.artist_info.name}`);
        logger.debug(
          `${data.album_list.length} albums, ${data.track_list.length} tracks`,
        );
      }
  ...
  const handleArtistClick = async (artist: {
    ...
    const artistUrl =
      artist.external_urls.replace(/\/$/, "") + "/discography/all";
  ...
  const handleConfirmAlbumFetch = async () => {
    ...
    try {
      ...
      const errorMsg =
        err instanceof Error ? err.message : "Failed to fetch album metadata";
      logger.error(`fetch failed: ${errorMsg}`);
      const settings = getSettings();
      if (!settings.useSpotFetchAPI) {
        setShowApiModal(true);
