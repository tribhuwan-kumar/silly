...
export function useLyrics() {
  const [downloadingLyricsTrack, setDownloadingLyricsTrack] = useState<
    string | null
  >(null);
  const [downloadedLyrics, setDownloadedLyrics] = useState<Set<string>>(
    new Set(),
  );
  ...
  const handleDownloadLyrics = async (
    ...
    try {
      ...
      toast.error(
        err instanceof Error ? err.message : "Failed to download lyrics",
      );
      setFailedLyrics((prev) => new Set(prev).add(spotifyId));
    } finally {
  ...
  const handleDownloadAllLyrics = async (
    ...
    for (let i = 0; i < tracksWithSpotifyId.length; i++) {
      ...
      try {
        ...
        if (settings.folderTemplate) {
          const folderPath = parseTemplate(
